...
module processor(halt, reset, clk);
  ...
  always @(posedge clk) begin
    case (s)
      ...
      `OPsys: //0
        begin
          if(doDec == 1)
            begin
              doDec = 0;
              //the rest of the stuff from the decode state
            
            end
          else
            begin
              //do the thing for the system call
              halt <= 1;
            end
        end
      ...
      `OPbrf: begin if(r[ir `SECOND4] == 0) begin pc <= pc + ir `BOTTOM8; end s <= `Start; end
      ...
      `OPlhi: begin r[if `SECOND4] `TOP8 <= ir `BOTTOM8; s <= `Start; end 
	...
	begin
	    ...
	    `OPmul: begin r[ir `DestReg] <= r[ir `DestReg] * r[ir `SourceReg]; s <= `Start; end
            `OPslt: begin r[ir `DestReg] <= (r[ir `DestReg] < r[ir `SourceReg] ? 16'b1 : 16'b0); end
